...
export default (Core, chunk, instruction) => {
  if (chunk.returnedData.length < 1) {
    ...
    if (instruction.operation === 'add') {
      ...
      } else return { error: true, content: `Cannot Perform "${['Is Greater', 'Is Greater Or Equal', 'Is Less', 'Is Less Or Equal'][['greater', 'greaterEqual', 'less', 'lessEqual'].indexOf(instruction.operation)]}" Operation On <${chunk.returnedData[0].type}> Using <${chunk.returnedData[1].type}>` }
      ...
      } else return { error: true, content: `Cannot Perform "${['And', 'Or'][['and', 'or'].indexOf(instruction.operation)]}" Operation On <${chunk.returnedData[0].type}> Using <${chunk.returnedData[1].type}>` }
      ...
      else return { error: true, content: `Cannot Perform "Not" Operation On <${chunk.returnedData[0].type}>` }
    }
